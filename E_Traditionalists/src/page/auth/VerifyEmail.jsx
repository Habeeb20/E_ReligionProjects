...
//     try {
//       const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/user/verify-email`, { email, code: otpCode }, {
...
//             className="w-full py-3 bg-gradient-to-r from-blue-500 to-blue-800 text-white font-semibold rounded-full hover:from-blue-600 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-e-ride-purple transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
//           >
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Redirect to Paystack payment with role-based amount
        const amount = role === "user" ? 5000 : role === "leader" ? 10000 : 15000; // Example amounts in kobo (e.g., 50, 100, 150 NGN)
        navigate(`/paystackpayment?email=${encodeURIComponent(email)}&role=${encodeURIComponent(role)}&amount=${amount}`);
        toast.success("Email verified successfully! Redirecting to payment...", {
          style: { background: "#4CAF50", color: "white" },
        });
      }
    } catch (err) {
