    ...
    const fetchMinisters = async () => {
      try {
        ...
        if (response.data.status && Array.isArray(response.data.data)) {
          ...
          console.log(response.data.data, "your details");
        } else {
  ...
  const MinisterCard = ({ minister }) => {
    ...
    return (
      <div className="border rounded-md p-4 shadow-md hover:shadow-lg transition duration-300 ease-in-out">
        ...
        <Link
         to={`/leader/${slug}`}className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">
          View More
        </Link>
      </div>
    );
    ...
    <div className="bg-gray-100 min-h-screen py-12 px-4 md:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {ministers.length > 0 ? (
          ...
          <p className="text-center text-gray-600">No ministers available.</p>
        )}
      </div>
